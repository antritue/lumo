...
import { deleteRoomService, updateRoomService } from "./route";
...
describe("DELETE /api/rooms/[id]/services/[serviceId]", () => {
	...
	it("should return 204 when authenticated user deletes a room service", async () => {
		...
		expect(mockEq).toHaveBeenCalledWith("room_id", ROOM_ID);
	...
	it("should return 404 when room service is not found", async () => {
		mockAuthenticatedUser();
		mockEq3.mockResolvedValue({
			error: null,
			count: 0,
		...
		expect(data.error).toBe("Room service not found");
	});
...
describe("PATCH /api/rooms/[id]/services/[serviceId]", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("should return 200 when updating pricing overrides", async () => {
		...
		const res = await updateRoomService(req, createParams(ROOM_ID, SVC_1_ID));
		const data = await res.json();
